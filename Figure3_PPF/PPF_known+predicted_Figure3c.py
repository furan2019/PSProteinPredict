...
Nops_df_org_real = pd.read_csv("NonPS_7features+PPF.csv", index_col=0)
Ps_df_org_real = pd.read_csv("PS_7features+PPF.csv", index_col=0)

Nops_df_org_pred = pd.read_csv("NonPS_7features+PPF_predicted.csv", index_col=0)
Ps_df_org_pred = pd.read_csv("PS_7features+PPF_predicted.csv", index_col=0)

Nops_df_org = pd.concat([Nops_df_org_real, Nops_df_org_pred])
Ps_df_org = pd.concat([Ps_df_org_real, Ps_df_org_pred])
...
for re10 in range(0, 10):
    ...
    Nops_df_re = Nops_all_filled.sample(n=len(Ps_df_org), random_state=re10)
...
Auu.to_csv("PPF_known+predicted_Figure3c.csv")
